...
void CALLBACK intervallMouseControl(
	...
	static Point* lastpoints[8] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
	...
	for (int i = 0; i < 8; i++){
		...
		if ((seenPoints[i] != NULL) && (seenPoints[i]->getX() >= minValue) && (seenPoints[i]->getX() <= maxValue) && (seenPoints[i]->getY() >= minValue) && (seenPoints[i]->getY() <= maxValue)) {
			pointNr++;
